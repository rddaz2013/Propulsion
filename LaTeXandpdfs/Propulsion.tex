...
\put(-3.7,-0.4){{ liquid $\dot{m}_{\text{Ox}}(0)$ }}
\put(0.1,0.0){{ $$ }}
...
\]
Using mass conservation on fuel, $\frac{d\dot{m}_g}{dx} = -\frac{d\dot{m}_g}{dx}$, so
\begin{equation}
...
\[
v_g = v_g(x) = \frac{m_g}{\rho_g A_{cc}} = \frac{\dot{m}_g(X) R_U T_g(x) }{ MW_g P A_{cc}} = \frac{ \left( \left( \frac{F}{O} \right)_{\Phi =1} \phi_g(x) + 1 \right) \dot{m}_{\text{Ox}}(0) R_U T_g(x) }{ MW_g P A_{cc}}
\]
...
\[
( \dot{m}_{\text{Ox}}(0), \mathcal{N}, \dot{m}_l(0) ) \in C^{\infty}(N_0) \times C^{\infty}(N_0) \times C^{\infty}(N_0)
\]
